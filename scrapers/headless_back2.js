...
async function scrapeMensuales(rfc, clave, year) {
    ...
    try {
        ...
        const downloadDec = await collectDataDec(page, rfc, 'dec')

        await page.waitFor(2000)

        await gotToSectionAcuse(page)
        await searchDec(page, year)
        const downloadAcuse = await collectDataDec(page, rfc, 'acuse')
...
async function collectDataDec(page, rfc, type) {
    ...
    const download = await exgtractPdfsDecNew(page, rfc, urlsPdf, type)
...
async function getPdfUrls(page, idsLinkDec, type) {
    ...
    for (let u = 0; u < idsLinkDec.length; u++) {
        ...
        urlsDec.push({
         ...
         if (urlsDec.length == 1 ||  u == idsLinkDec.length -1) {
            urlsDec.push({
                operacion,
                pdf: pdfSAT
...
async function exgtractPdfsDec(page, rfc, idsLinkDec, type) {
    // await page.setRequestInterception(true)

    await page.setMaxListeners(100);
    ...
    for (var u = 0; u < idsLinkDec.length; u++) {
        ...
        try {
            ...
            page.on('request', (interceptedRequest) => {
                ...
                console.log('req:', interceptedRequest.url())
                ...
                if (interceptedRequest.method() === "POST") {
                    ...
                    if (requestData.indexOf('ConsultasDeclaracion') == -1) {
                        ...
                        if (pdfEncode) {
                            pdfsEncodeDownloaded.push(`${type}_${rfc}_${operacion}.txt`)
                            page.evaluate('document.querySelector("#btnBack").click()')
